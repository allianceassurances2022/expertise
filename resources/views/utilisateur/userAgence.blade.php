...
@section('head_title')
Afféctation Agence à Utilisateur
@endsection

@section('title')
Afféctation des Agences à l'Utilisateur N° {{$user->id}} ({{$user->username}})
@endsection
...
@section('head')
	...
	<style type="text/css">
		...
		/*table#dejaaffecte th:nth-child(3), table#dejaaffecte tbody td:nth-child(3){
...
<div class="row">
	...
	<div class="col-lg-4 col-md-6 col-sm-12 mb-3">
		<div class="card">
			...
			<div class="card-body collone">
				<table id="pourAffecterAgenceUser" class=" table dataTable table-responsive table-hover  display  nowrap" style="border-bottom:1px #0000004d solid;height: 450px; overflow: auto; width:100%;">
			        <thead class="nowrap" >
			            <tr>
			                <th class="all" style="width:43% !important;">DR</th>
			                <th class="all" >CODE</th>
			                <th class="all" >STATUT</th>
			            </tr>
			        </thead>
	...
	<div class="col-lg-4 col-md-6 col-sm-12 mb-3">
		<div class="card">
			<div class="card-header dejaAffecter">
				<h4 class="header-title"> <span class="fa fa-home"></span> Liste des Agences deja affectées</h4>
			</div>
			<div class="card-body collone">
				<table id="dejaAffecte" class=" table table-responsive display table-hover nowrap" style="border-bottom:1px #0000004d solid;height: 400px; overflow: auto;">
			        <thead>
			            <tr>
			                <th style="width: 10% !important;">DR</th>
			                <th style="width: 10% !important;">CODE</th>
			                <th style="width: 200px !important;">DATE D'AFFECTATION</th>
			            </tr>
			        </thead>
			        <tbody>
			        	@foreach($agence_list as $agence)
			        	<tr id="{{$agence->id}}" class="nowrap">
				        	<td>{{$agence->dr}}</td>
				        	<td>{{$agence->code}}</td>
			...
			<form id="frmDesaffecterListe" action="{{route('user.desaffecteAgenceUser')}}">
				@csrf
				<div class="col-xs-12 text-right" style="margin:5px 19px 22px 4px;">
					<button id="btnAffecterListe" type="submit" class="btn btn-danger waves-effect waves-light">Désaffecter la liste selectionnée</button>
				</div>
			</form>
		</div>
	</div>
...
function toggleRow(row){
  	 ...
  	 var row_id =  $(row).attr('id');
    	if(jQuery.inArray(row_id, items) != -1 ){ //element_id exist  => so remove it
		     $('#pourAffecterAgenceUser tbody tr').remove('#'+row_id);
		}else{ // element_id dosen't exist  => so add it
    		 $('#pourAffecterAgenceUser tbody').prepend(row);
		}
}
    ...
    if (items.length !== 0) {
	    $.ajax({
	        ...
	        success: function(data) {
	                   ...
	                   window.location.href = "{{route('user.affecteAgence',$user->id )}}";
